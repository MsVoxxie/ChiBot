...
module.exports = {
	...
	async execute(bot, message, args, settings) {
		...
		if (!args.length) return message.lineReply(`Usage: ${settings.prefix}remove <Queue Number>`).then(s => s.delete({ timeout: 30 * 1000 }));
		if (isNaN(args[0])) return message.lineReply(`Usage: ${settings.prefix}remove <Queue Number>`).then(s => s.delete({ timeout: 30 * 1000 }));
