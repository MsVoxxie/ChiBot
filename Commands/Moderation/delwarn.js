...
module.exports = {
	...
	async execute(bot, message, args, settings) {
		...
		// Declarations
		const warns = JSON.parse(readFileSync(path.join(__dirname, '../../DataStore/Warnings/', './Warnings.json'), 'utf8'));

		const delWarn = (message.mentions.members.first() || message.guild.members.cache.get(args[0]));

		// Checks
		if (!delWarn) message.lineReply('\nPlease provide a user to remove from the warning list.').then(s => s.delete({ timeout: 30 * 1000 }));

		console.log(delWarn.id);

		// Setup "Database"
		if (!warns[message.guild.id]) { warns[message.guild.id] = {}; }
		if (!warns[message.guild.id][delWarn.id]) return message.lineReply('\nThis user has not been warned.').then(s => s.delete({ timeout: 30 * 1000 }));
