...
module.exports = {
	...
	async execute(bot, message, args, settings) {
		...
		// Setup variables
		const toMute = message.guild.member(message.mentions.members.first() || message.guild.members.cache.get(args[0]));

		// Checks
		if (!toMute) return message.lineReply('\nSpecify a user who to mute.').then(s => s.delete({ timeout: 30 * 1000 }));
		if (toMute.hasPermission('MUTE_MEMBERS')) return message.lineReply('\nSorry you can\'t mute this user.').then(s => s.delete({ timeout: 30 * 1000 }));
